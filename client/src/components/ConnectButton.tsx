...
import { EIP712_DOMAIN, EIP712_TYPES, PRIMARY_TYPE } from "../config/web3";

interface ConnectButtonProps {
    onSuccessLogin?: () => void;
    onGenerateNonce?: (address: string) => Promise<any>;
    onVerifyAndLogin?: (address: string, signature: string) => Promise<any>;
...
export const ConnectButton: React.FC<ConnectButtonProps> = ({
        ...
        try {
            ...
            try {
                signature = await signTypedDataAsync({
                    ...
                    message: {
                        wallet: targetAddress as `0x${string}`,
                        nonce: nonce,
                    },
                });
            } catch (typedErr: any) {
    ...
    if (isConnected && address && showDisconnect) {
        return (
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                <button type="button" className={className} onClick={handleWalletAuth} disabled={loading}>
    ...
    return (
        <button type="button" className={className} onClick={handleWalletAuth} disabled={loading || isConnectPending}>
